...
先通过地层属性创建一个数值模拟模型，通过数值模拟模型预测未来时间步的产油等生产数据，然后通过对应的实际的生产数据与预测数据的对比，进一步优化数值模拟模型的预测精度，然后通过不同的注采参数，分别进行数值模拟，并得出不同注采参数对应的生产数据，通过对比得出预测的时间步内累计产油量最高的所对应的注采参数，这时，即为最优注采策略，然后再次通过应用次注采策略的实际生产数据进一步优化数值模型，以此达到闭环。
